...
public class AntDoc
{
		...
		for (int i=0;i<meth.length;i++) {
			...
			if (!name.equalsIgnoreCase("add") && !name.equalsIgnoreCase("addtask")) {
				if (isTaskAddingFunction(meth[i])) {
					System.out.println("SPECIAL NESTED:"+name);
	...
	private boolean isTaskAddingFunction(MethodDoc m) {		
		String name=m.name();
		if (!name.startsWith("add")) return false;		
		Parameter[] pt=m.parameters();
		if (pt==null) return false;			
		ClassDoc firstParam=pt[0].type().asClassDoc();
		if (this.isSubClassOfBase(firstParam)) {
			return true;
		}		
		return false;
	...
	public boolean isBaseTask(ClassDoc sd) {
		for (int i=0;i<m_baseclassNames.length;i++) {
			if (sd.qualifiedName().equalsIgnoreCase(m_baseclassNames[i])) return true;
		}
		return false;
	}
	
	public boolean isSubClassOfBase(ClassDoc d) {
		if (isBaseTask(d)) return true;
		for (int i=0;i<m_baseclassNames.length;i++) {
			if (d.subclassOf(m_Root.classNamed(m_baseclassNames[i]))) return true;
		}
		return false;
